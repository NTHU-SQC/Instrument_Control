# -*- coding: utf-8 -*-
"""
Created on Wed Jun 30 10:53:53 2021
...
class AWG5208(Instrument):
    ...
    def __str__(self):
        ...
        """
        print(f'{self.instName} @ {self.address}\n' +
              '---\n'
              f'Waveform list: {self.wfmList}\n' +
              f'Sequence list: {self.seqList}')
        return 'AWG Error state: %s' % (self.inst.query('system:error:all?'))
    ...
    def set_wfm(self,
        ...
        self.inst.write(f'WLIST:WAVEFORM:NEW "{wfm_name}", {wfm_length}')
    ...
    def upload_wfm(self):
        ...
        for wfm_name in self._wfmList.keys():
            ...
            stringArg = 'WLIST:WAVEFORM:DATA ' + \
                f'"{wfm_name}", 0, {int(wfm_length)}, '
            # stringArg = 'WLIST:WAVEFORM:DATA "%s", 0, %d, ' % (
            #     wfm_name, wfm_length)
    ...
    def assign_ch(self, ch, name, track_index=None,
        ...
        self.inst.write(command + mode + suffix)
    ...
    def set_ch_amp(
        ...
        **kwargs : str
        ...
        for i, volt in zip(range(self.__class__.MAX_MKR_NUM, 0, -1), [
            ...
            value = f'{volt}V'
            mid = f':MARKER{i}'
            self.inst.write(command + mid + suffix + value)
            self.inst.write('*opc')
        if wfm_Vpp:
            self.inst.write(command + suffix + f'{wfm_Vpp}')
            self.inst.write('*opc')
        if 'dBm' in kwargs.keys():
            suffix = ':POWER:LEVel:IMMediate:AMPLITUDE '
            self.inst.write(command + suffix + f'{kwargs["dBm"]}')
            self.inst.write('*opc')
        if wfm_offset:
            self.inst.write(
    ...
    def set_ch_output(self, output, *args):
        ...
        *args : int
        ...
        for ch in args:
            if output:
                self.inst.write(f'OUTPut{ch}:STATE ON')
                continue
            self.inst.write(f'OUTPut{ch}:STATE OFF')
